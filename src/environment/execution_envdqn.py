...
class OptimalExecutionEnv(gym.Env):
    ...
    def __init__(
        ...
        self.w_twap = 0.8        # pousse à battre TWAP (shaping)
        self.w_impact = 2.0      # pénalise l'impact (slippage)
        self.w_downside = 0.4    
        self.w_hold = 0       # pression temporelle sur inventaire restant
        self.w_bonus = 0.05      # bonus pour vendre >= prix initial

        self.reward_scale = 10000.0   # en "bps" du portfolio
        self.reward_clip = 20.0       # clipping pour stabilité DQN
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, dict]:
        ...
        if self.sequential_mode:
            ...
            use_random_start = rng.random() < self.random_start_prob
            
            if use_random_start:
                self.random_start_idx = int(self.np_random.integers(min_start, max_start))
                self.current_step = int(self.np_random.integers(0, self.horizon_steps))
                random_time_elapsed_ratio = self.current_step / self.horizon_steps
                inventory_sold_ratio = random_time_elapsed_ratio * float(self.np_random.uniform(0.8, 1.2))
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        # (A) Impact cost: how much you "paid" versus mid due to temporary impact
        impact_cost = max(0.0, (current_price - execution_price)) * agent_quantity  # >= 0

        # (B) Downside penalty: ONLY if you executed below the initial (arrival) price
        downside_cost =  abs((self.initial_price - execution_price)) * agent_quantity

        # (C) Bonus for selling at/above initial price (encourage "good" fills)
        sell_bonus = (execution_price >= self.initial_price) * (self.initial_price * agent_quantity)

        # (D) Time pressure (after you trade): penalize holding inventory as time passes
        inventory_after = self.inventory - agent_quantity
        time_frac_after = (self.current_step + 1) / self.horizon_steps   # in [0,1]
        hold_penalty = self.initial_price * max(0.0, inventory_after) * (time_frac_after ** 2)

        # (E) Optional: advantage vs TWAP at the same moment (reward shaping)
        twap_advantage = agent_revenue - twap_revenue

        reward_dollar = (
            + self.w_twap * twap_advantage
            - self.w_impact * impact_cost
            - self.w_downside * downside_cost
            - self.w_hold * hold_penalty
            + self.w_bonus * sell_bonus
        )

        reward = (reward_dollar / portfolio_value) * self.reward_scale
        ...
        if terminated and self.inventory > 0:
            ...
            # impact cost on the forced liquidation
            fs_impact_cost = max(0.0, (final_price - fire_sale_price)) * remaining_qty

            # downside penalty only (below initial)
            fs_downside_cost = abs((self.initial_price - fire_sale_price)) * remaining_qty

            # explicit lateness penalty: being forced to dump at horizon is bad
            fs_lateness_penalty = self.initial_price * remaining_qty * 0.10

            fs_reward_dollar = (
                - self.w_impact * fs_impact_cost
                - self.w_downside * fs_downside_cost
                - fs_lateness_penalty
            )

            reward += (fs_reward_dollar / portfolio_value) * self.reward_scale
            reward = float(np.clip(reward, -self.reward_clip, self.reward_clip))
